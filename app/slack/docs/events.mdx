...
`slack.event` signal. When `workflow.definitionId` is configured, the request
references that global workflow definition and omits an inline target. Later
Slack messages in the same thread signal the existing keyed run instead of
replacing its target or authorization context. Set positive
...
definition instead of generating agent, session-started, or reply steps. Use
...
Set `slackReply.agentOutput` to deliver that step output through
`slack.events.reply`.
