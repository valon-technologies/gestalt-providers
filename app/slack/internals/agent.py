...
from .models import SlackEventDeliveryConfig as SlackEventDeliveryConfig  # noqa: F401
...
from .reply_ref import (
    agent_session_ref as _agent_session_ref,
    decode_interaction_ref as _decode_interaction_ref_impl,
    reply_ref_workflow_key as _reply_ref_workflow_key,
    sign_interaction_ref as _sign_interaction_ref_impl,
    sign_reply_ref as _sign_reply_ref_impl,
    verify_interaction_ref as _verify_interaction_ref_impl,
    verify_reply_ref as _verify_reply_ref_impl,
)
from .reply_ref_auth import (
    SLACK_AGENT_WORKFLOW_EVENT_TYPE,
    SLACK_INTERACTION_WORKFLOW_EVENT_TYPE,
    authorize_reply_ref,
)
...
def resolve_slack_http_subject(
    ...
    deliver_event, _deliver_ignored_reason = _slack_deliver_callback_from_payload(
        payload
    )
    has_deliver_route = bool(
...
def _log_ignored_slack_event(
    ...
    if not (_agent_config.routes or _agent_config.events.deliver.routes):
        return
...
def _event_reply_ref(reply_ref: str, req: gestalt.Request) -> SlackReplyRef:
    ...
    return authorize_reply_ref(
        reply_ref,
        req,
        signing_key=_reply_ref_signing_key(),
    )
...
def _deliver_matching_workflow_events(
    payload: dict[str, Any], req: gestalt.Request
) -> dict[str, Any] | ErrorResponse | None:
    event, ignored_reason = _slack_deliver_callback_from_payload(payload)
    if event is None:
        _ = ignored_reason
        return None

    routes = _matching_deliver_routes(event)
    if not routes:
        return None
...
def _slack_deliver_callback_from_payload(
    payload: dict[str, Any],
) -> tuple[SlackEventDeliveryCallback | None, str]:
    callback_type = str(payload.get("type") or "").strip()
    if callback_type == SlackCallbackType.URL_VERIFICATION:
        return None, "url_verification"
    if callback_type != SlackCallbackType.EVENT_CALLBACK:
...
def _matching_deliver_routes(
    event: SlackEventDeliveryCallback,
) -> tuple[SlackEventDeliveryRoute, ...]:
    return tuple(
        route
        for route in _agent_config.events.deliver.routes
...
def _slack_deliver_event_data(
    ...
    return {
        ...
        "slack": {
            ...
            "file_ids": _deliver_event_file_ids(event),
...
def _deliver_event_file_ids(event: SlackEventDeliveryCallback) -> list[str]:
    return [
        file_id
        for file_id in (
...
def _slack_deliver_log_context(
    event: SlackEventDeliveryCallback,
    ...
    return _log_context(
        ...
        slack_message_ts=event.message_ts,
        slack_thread_ts=event.thread_ts,
        slack_deliver_route_ids=",".join(route.id for route in routes),
    )
...
def _sign_reply_ref(
    ...
    return _sign_reply_ref_impl(
        event,
        subject_id,
        route,
        signing_key=_reply_ref_signing_key(),
        ttl_seconds=SLACK_REPLY_REF_TTL_SECONDS,
    )


def _verify_reply_ref(reply_ref: str, subject_id: str) -> SlackReplyRef:
    return _verify_reply_ref_impl(
        reply_ref,
        subject_id,
        signing_key=_reply_ref_signing_key(),
    )
...
def _sign_interaction_ref(
    ...
    *,
    ...
    return _sign_interaction_ref_impl(
        ref,
        action_id=action_id,
...
def _verify_interaction_ref(
    ...
    return _verify_interaction_ref_impl(
        interaction_ref,
        subject_id,
        signing_key=_reply_ref_signing_key(),
    )
...
def _decode_interaction_ref(interaction_ref: str) -> SlackInteractionRef:
    return _decode_interaction_ref_impl(
        interaction_ref,
        signing_key=_reply_ref_signing_key(),
    )
...
def _slack_interaction_deliver_event_data(
    ...
    return {
        ...
        "slack": {
            ...
            "message_ts": interaction_ref.message_ts,
            "container_message_ts": string_field(container, "message_ts"),
