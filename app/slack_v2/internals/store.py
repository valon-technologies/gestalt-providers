...
DEFAULT_EVENT_REGISTRATION_ID = "default"
...
def get_default_workflow_event_subject() -> str:
    record = _object_store(EVENT_REGISTRATION_OBJECT_STORE_NAME).get(
        DEFAULT_EVENT_REGISTRATION_ID
    )
    return _workflow_event_subject_from_record(
        record, app_id=DEFAULT_EVENT_REGISTRATION_ID
    )


def get_default_signing_secret() -> str:
    record = _object_store(EVENT_REGISTRATION_OBJECT_STORE_NAME).get(
        DEFAULT_EVENT_REGISTRATION_ID
    )
...
def save_slack_event_registration(
    *,
    ...
    _object_store(EVENT_REGISTRATION_OBJECT_STORE_NAME).put(
        {"id": DEFAULT_EVENT_REGISTRATION_ID, "app_id": app_id, **record}
    )
...
def get_debug_payload(*, event_id: str) -> dict[str, Any]:
    ...
    if not isinstance(payload, dict):
        raise gestalt.NotFoundError(
            f"debug payload not found for event_id {event_id!r}"
        )
...
def _workflow_event_subject_from_record(record: Any, *, app_id: str) -> str:
    workflow_event_subject = record.get("workflow_event_subject")
    if (
        not isinstance(workflow_event_subject, str)
        or not workflow_event_subject.strip()
    ):
        raise gestalt.NotFoundError(
            f"workflow_event_subject not found for app_id {app_id!r}"
        )
    return workflow_event_subject.strip()


def _signing_secret_from_record(record: Any, *, app_id: str) -> str:
