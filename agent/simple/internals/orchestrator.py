...
class SimpleAgentOrchestrator:
    ...
    def create_turn(
        ...
        checkpoint = self._store.ensure_turn_checkpoint_from_seed(started)
        if checkpoint is None:
            self._fail_run(
                run=started,
                messages=list(started.messages),
                status_message="turn was created without a durable resume seed",
            )
            return self.turn_to_proto(self._store.get_turn(turn_id) or started)
    ...
    def _run_turn_loop(self, turn_id: str) -> None:
        try:
            ...
            if run is not None and run.status not in _TERMINAL_STATUSES:
                self._fail_run(
                    run=run, messages=list(run.messages), status_message=f"run exceeded maxSteps ({self._config.max_steps})"
                )
    ...
    def _run_model_step(self, *, run: StoredRun, checkpoint: StoredTurnCheckpoint) -> bool:
        ...
        if step.tool_calls:
            ...
            self._store.put_turn_checkpoint(next_checkpoint)
    ...
    def _continue_pending_tool(self, *, run: StoredRun, checkpoint: StoredTurnCheckpoint) -> bool:
        ...
        if not tool_call_id or not tool_name or not resolved_tool_id:
            self._fail_run(run=run, messages=checkpoint.messages, status_message="checkpoint is missing pending tool call")
            return False
        tool_specs, function_name_to_tool_id, loaded_tool_ids = _tool_registry_from_checkpoint(checkpoint)
        ...
        if checkpoint.phase == PHASE_REPAIR_ARGUMENTS:
            ...
            self._store.put_turn_checkpoint(checkpoint)
        ...
        if checkpoint.phase == PHASE_TOOL_RESULT_RECORDED:
            ...
            if result is None:
                self._fail_run(run=run, messages=checkpoint.messages, status_message="checkpoint references missing tool result")
                return False
            return self._record_tool_result_in_conversation(checkpoint=checkpoint, result=result)

        validation_error = _tool_arguments_validation_error(tool_name=tool_name, arguments=arguments, tool_specs=tool_specs)
        ...
        self._store.put_turn_checkpoint(inflight)
        if validation_error:
            result = {"status": 400, "body": validation_error, "is_error": True, "tool_id": resolved_tool_id}
            self._store.put_tool_result(turn_id=checkpoint.turn_id, tool_call_id=tool_call_id, result=result)
            self._store.append_turn_event_once(
                event_key=f"{checkpoint.turn_id}:tool:{tool_call_id}:completed",
                turn_id=checkpoint.turn_id,
                event_type="tool.completed",
                source=checkpoint.provider_name,
                data={"tool_call_id": tool_call_id, "tool_id": resolved_tool_id, "status": 400, "error": validation_error},
            )
            return self._record_tool_result_in_conversation(checkpoint=replace(inflight, phase=PHASE_TOOL_RESULT_RECORDED), result=result)
        ...
        if resolved_tool_id == TOOL_SEARCH_TOOL_ID:
            ...
            return self._record_tool_result_in_conversation(checkpoint=replace(updated, phase=PHASE_TOOL_RESULT_RECORDED), result=result)
    ...
    def _fail_run(self, *, run: StoredRun, messages: list[dict[str, Any]], status_message: str) -> None:
        cleaned_status_message = status_message.strip()
        self._store.mark_turn_failed(turn_id=run.run_id, messages=messages, status_message=cleaned_status_message)
        try:
            self._store.append_turn_event_once(
...
def _uncertain_tool_status_message(pending_tool_call: dict[str, Any] | None) -> str:
    ...
    return f"tool call {tool_call_id} may have executed before provider restart; refusing to replay without host idempotency"
...
def _checkpoint_messages(checkpoint: StoredTurnCheckpoint | None, *, fallback: list[dict[str, Any]]) -> list[dict[str, Any]]:
