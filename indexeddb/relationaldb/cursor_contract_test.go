...
func (h *relationalContractHarness) InsertUnreadablePayloadRow(t *testing.T, storeName, id, status string) {
	...
	store, err := newStoreWithOptions(context.Background(), h.dsn, storeOptions{TablePrefix: h.prefix})
	if err != nil {
	...
	if _, err := store.db.ExecContext(context.Background(), store.q(indexStmt), storeName, "by_status", indexKey.hash, indexKey.raw, indexKey.ord, primary.hash, primary.raw, primary.ord); err != nil {
