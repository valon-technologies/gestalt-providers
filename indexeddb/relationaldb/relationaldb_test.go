...
func TestFullLifecycle(t *testing.T) {
	...
	createdAt, err := wirecodec.AnyFromTypedValue(resp.Record.Fields["created_at"])
	if err != nil {
	...
	// Index query.
	vals, _ := wirecodec.TypedValuesFromAny([]any{"W-001"})
...
func TestGenericStoreSupportsTypedPrimaryKeyLookup(t *testing.T) {
	...
	id, err := wirecodec.AnyFromTypedValue(resp.Record.Fields["id"])
	if err != nil {
...
func mustTypedValue(t *testing.T, value any) *proto.TypedValue {
	t.Helper()
	pbValue, err := wirecodec.TypedValueFromAny(value)
	if err != nil {
