...
func encodeOrderedNumberValue(value any) ([]byte, error) {
	...
	f, _ := rat.Float64()
	if math.IsInf(f, 0) {
		return nil, status.Error(codes.InvalidArgument, "number key magnitude out of range")
	}
	...
	out = append(out, 0x01)
	out = append(out, encodeRatULPOffset(diff)...)
	return out, nil
}
...
func encodeRatULPOffset(offset *big.Rat) []byte {
	...
	den := offset.Denom()
	numMag := encodeLengthPrefixedMag(num.Bytes())
	denMag := encodeLengthPrefixedMag(den.Bytes())
...
func encodeLengthPrefixedMag(mag []byte) []byte {
	if len(mag) == 0 {
		mag = []byte{0x00}
	}
...
// numberRat and floatRat mirror sdk/go/internal/indexeddbcodec/compare_keys.go;
// keep them in sync or CompareKeys and encodeOrderedKey will drift apart silently.
