...
class FakeHTTPResponse:
    def __init__(
        self, body: str = "", headers: dict[str, str] | None = None, status: int = 200
    ) -> None:
...
class VercelProviderTests(unittest.TestCase):
    ...
    def test_blob_put_requires_blob_token(self) -> None:
        ...
        self.assertEqual(
            response.body, {"error": "blobReadWriteToken is not configured"}
        )
    ...
    def test_blob_put_passes_configured_blob_token(self) -> None:
        ...
        with mock.patch.object(
            provider_module,
            "put_blob",
            return_value={"data": {"blob": {"pathname": "roadmaps/newrez.json"}}},
        ) as put_blob:
            result = provider_module.blob_put(
                provider_module.BlobPutInput(
                    pathname="roadmaps/newrez.json",
    ...
    def test_blob_put_rejects_invalid_access(self) -> None:
        ...
        self.assertEqual(
            response.body, {"error": "access must be either private or public"}
        )
    ...
    def test_blob_head_maps_not_found_error(self) -> None:
        ...
        with mock.patch.object(
            ...
            side_effect=VercelBlobAPIError(
                404, "Vercel Blob: The requested blob does not exist"
            ),
        ):
            result = provider_module.blob_head(
                provider_module.BlobHeadInput(url_or_path="roadmaps/missing.json")
            )
        ...
        self.assertEqual(
            response.body, {"error": "Vercel Blob: The requested blob does not exist"}
        )
    ...
    def test_blob_get_maps_access_error(self) -> None:
        ...
        with mock.patch.object(
            ...
            result = provider_module.blob_get(
                provider_module.BlobGetInput(
                    url_or_path="roadmaps/newrez.json", access="private"
                )
            )
    ...
    def test_blob_get_rejects_non_positive_timeout(self) -> None:
        result = provider_module.blob_get(
            provider_module.BlobGetInput(
        ...
        self.assertEqual(
            response.body,
            {"error": "timeout_seconds must be positive when provided"},
        )

    def test_blob_copy_maps_invalid_destination_path(self) -> None:
        provider_module.configure(
            "vercel",
            {
                "clientId": "client-id",
        ...
        result = provider_module.blob_copy(
            provider_module.BlobCopyInput(
                source_url_or_path="https://store.blob.vercel-storage.com/source.txt",
        ...
        response = cast(gestalt.Response[dict[str, str]], result)
        self.assertEqual(response.status, HTTPStatus.BAD_REQUEST)
        self.assertEqual(response.body, {"error": 'pathname cannot contain "//"'})

    def test_blob_delete_requires_targets(self) -> None:
        result = provider_module.blob_delete(
            provider_module.BlobDeleteInput(targets=[])
        )
        ...
        self.assertEqual(
            response.body,
            {"error": "targets must contain at least one non-empty value"},
        )
    ...
    def test_blob_delete_calls_delete_blobs_with_trimmed_targets(self) -> None:
        ...
        with mock.patch.object(
            provider_module, "delete_blobs", return_value={"data": {"deleted": 2}}
        ) as delete_blobs:
            result = provider_module.blob_delete(
                provider_module.BlobDeleteInput(
    ...
    def test_put_blob_uses_vercel_blob_put_contract(self) -> None:
        def fake_urlopen(
            ...
            self.assertEqual(
                authorization_header(request), "Bearer blob_rw_x_store123_token"
            )
        ...
        with mock.patch.object(
            blob_module.urllib.request, "urlopen", side_effect=fake_urlopen
        ):
    ...
    def test_delete_blobs_uses_delete_contract(self) -> None:
        def fake_urlopen(
            ...
            self.assertEqual(
                request.data, b'{"urls": ["roadmaps/a.json", "roadmaps/b.json"]}'
            )
            return FakeHTTPResponse()
