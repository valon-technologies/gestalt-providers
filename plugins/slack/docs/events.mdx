...
Configure `assistantLedger` when a Slack assistant should either post a final
reply or a Slack-visible failure message after accepting a request. The ledger is
a thin Slack-side record: it tracks the source Slack event, acknowledgement
errors, reply delivery success/failure, recovery attempts, and terminal status.
Workflow run, signal, idempotency, and status state remain owned by the workflow
provider.
...
workflows:
  schedules:
    slack_assistant_reconcile_stuck_requests:
      provider: local
      cron: "*/5 * * * *"
...
`assistantLedger.indexeddb` selects the IndexedDB provider instance used for the
Slack-side ledger. The object store defaults to `slack_assistant_requests`; omit
`store` unless an existing deployment needs a different store name. The
reconciler retries stale non-terminal records by re-signaling the workflow, and
posts the configured fallback message when reply delivery failed or recovery
...
`events.addReaction`, `events.removeReaction`, `assistant.reconcileStuckRequests`,
the native assistant helpers, and the interaction helpers are hidden operations.
