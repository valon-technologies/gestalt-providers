...
channel, use `eventTypes: [message.channels]` on that route. Message routes can
set `subtypes`: omitted means all non-ignored subtypes can match,
`subtypes: []` means only normal messages with no subtype match, and a non-empty
list matches those Slack message subtypes. Bot, edit, delete, and thread-reply
