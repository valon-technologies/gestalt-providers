...
func TestProviderExecutionReferenceRoundTripsAgentTarget(t *testing.T) {
	...
	if !reflect.DeepEqual(got.Target, ref.Target) {
...
func TestProviderAgentSchedulePersistsTargetAndInvokesHost(t *testing.T) {
	...
	if len(listed.Schedules) != 1 || !reflect.DeepEqual(listed.Schedules[0].Target, schedule.Target) {
	...
	if !reflect.DeepEqual(run.Target, call.Target) {
...
func startTestIndexedDBBackendWithWrapper(t *testing.T, wrap func(gestalt.IndexedDBProvider) gestalt.IndexedDBProvider) {
	...
	_ = lis.Close()
	indexedDBProvider := gestalt.IndexedDBProvider(store)
	if wrap != nil {
		indexedDBProvider = wrap(indexedDBProvider)
	}
	ctx, cancel := context.WithCancel(context.Background())
	t.Setenv("GESTALT_PLUGIN_SOCKET", socketPath)
	go func() { _ = gestalt.ServeIndexedDBProvider(ctx, indexedDBProvider) }()
	waitForCondition(t, time.Second, func() bool {
...
func startTestWorkflowHost(t *testing.T, host *workflowHostStub) {
	t.Helper()
	previous := workflowHostConnector
	workflowHostConnector = func() (workflowHost, error) {
		return host, nil
	}
	t.Cleanup(func() {
		workflowHostConnector = previous
	})
}
