...
func TestProviderSignalWakePrefersRunAndBatchesSignals(t *testing.T) {
	...
	startProviderWorker(t, provider)
	call, err := host.waitForCall(10 * time.Second)
	if err != nil {
...
func TestProviderExecutionReferencesRoundTripAndListBySubject(t *testing.T) {
	...
	if listed.GetReferences()[0].ID != "ref-1" || listed.GetReferences()[1].ID != "ref-2" {
		t.Fatalf("subject references ids = [%s %s], want [ref-1 ref-2]", listed.GetReferences()[0].ID, listed.GetReferences()[1].ID)
	}
...
func TestNormalizeTargetRejectsSessionReadyDeliveryInvalidSources(t *testing.T) {
	target := protoAgentTargetWithOutputDelivery("managed", "gpt-5.4", "send a Slack reminder")
	target.Agent.SessionReadyDelivery = protoSessionReadyDelivery()
...
func TestProviderExecutionReferenceRoundTripsAgentTarget(t *testing.T) {
	...
	if targetJSON(got.Target) != targetJSON(ref.Target) {
...
func TestProviderPublishEventAgentTargetExecutionReferenceIncludesOutputDelivery(t *testing.T) {
	...
	ref, err := provider.GetExecutionReference(ctx, &gestalt.GetWorkflowExecutionReferenceRequest{ID: runs.GetRuns()[0].ExecutionRef})
	if err != nil {
		t.Fatalf("GetExecutionReference: %v", err)
	}
...
func TestProviderAgentSchedulePersistsTargetAndInvokesHost(t *testing.T) {
	...
	if len(listed.GetSchedules()) != 1 || targetJSON(listed.GetSchedules()[0].Target) != targetJSON(schedule.Target) {
	...
	if targetJSON(run.Target) != targetJSON(call.Target) {
...
func TestProviderLeavesAgentToolValidationToHost(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			target: protoAgentTargetFromMessage(&gestalt.BoundWorkflowAgentTarget{}),
		},
		{
			name:   "empty prompt",
			target: protoAgentTargetFromMessage(&gestalt.BoundWorkflowAgentTarget{ProviderName: "managed"}),
		},
		{
...
func (s *workflowHostStub) InvokeOperation(ctx context.Context, req gestalt.InvokeWorkflowOperationInput) (*gestalt.InvokeWorkflowOperationResponse, error) {
	cloned := cloneInvokeWorkflowOperationInput(req)
	s.mu.Lock()
...
func (s *workflowHostStub) waitForCall(timeout time.Duration) (gestalt.InvokeWorkflowOperationInput, error) {
	...
	case <-time.After(timeout):
		return gestalt.InvokeWorkflowOperationInput{}, context.DeadlineExceeded
	}
}
...
func cloneInvokeWorkflowOperationInput(input gestalt.InvokeWorkflowOperationInput) gestalt.InvokeWorkflowOperationInput {
	return gestalt.InvokeWorkflowOperationInput{
		Target:       cloneTarget(input.Target),
		RunID:        input.RunID,
		Trigger:      cloneRunTrigger(input.Trigger),
		Input:        cloneAny(input.Input),
		Metadata:     cloneAny(input.Metadata),
		CreatedBy:    cloneActor(input.CreatedBy),
		ExecutionRef: input.ExecutionRef,
		Signals:      cloneSignalValues(input.Signals),
	}
}

func cloneRunTrigger(trigger *gestalt.WorkflowRunTrigger) *gestalt.WorkflowRunTrigger {
	if trigger == nil {
		return nil
	}
	out := &gestalt.WorkflowRunTrigger{Manual: trigger.Manual}
	if trigger.Schedule != nil {
		out.Schedule = &gestalt.WorkflowScheduleTrigger{
			ScheduleID:   trigger.Schedule.ScheduleID,
			ScheduledFor: timePtrOrNil(trigger.Schedule.ScheduledFor),
		}
	}
	if trigger.Event != nil {
		out.Event = &gestalt.WorkflowEventTriggerInvocation{
			TriggerID: trigger.Event.TriggerID,
			Event:     cloneEvent(trigger.Event.Event),
		}
	}
	return out
}

func cloneSignalValues(values []gestalt.WorkflowSignal) []gestalt.WorkflowSignal {
	if len(values) == 0 {
		return nil
	}
	out := make([]gestalt.WorkflowSignal, 0, len(values))
...
func startTestIndexedDBBackendWithWrapper(t *testing.T, wrap func(gestalt.IndexedDBProvider) gestalt.IndexedDBProvider) {
	...
	go func() { _ = gestalt.ServeIndexedDBProvider(ctx, indexedDBServer) }()
	waitForCondition(t, time.Second, func() bool {
...
func startTestWorkflowHost(t *testing.T, host workflowHostClient) {
	t.Helper()
	previous := workflowHostFactory
	workflowHostFactory = func() (workflowHostClient, error) {
		return host, nil
	}
	t.Cleanup(func() {
		workflowHostFactory = previous
	})
}

func protoBoundTarget(t *testing.T, pluginName, operation string, input map[string]any) *gestalt.BoundWorkflowTarget {
	t.Helper()
...
func protoAgentTarget(providerName, model, prompt string) *gestalt.BoundWorkflowTarget {
	return protoAgentTargetFromMessage(&gestalt.BoundWorkflowAgentTarget{
...
func protoAgentTargetWithOutputDelivery(providerName, model, prompt string) *gestalt.BoundWorkflowTarget {
...
func protoAgentTargetWithDeliveries(providerName, model, prompt string) *gestalt.BoundWorkflowTarget {
	target := protoAgentTargetWithOutputDelivery(providerName, model, prompt)
	target.Agent.SessionReadyDelivery = protoSessionReadyDelivery()
	return target
}

func protoSessionReadyDelivery() *gestalt.WorkflowOutputDelivery {
...
func protoAgentTargetFromMessage(agent *gestalt.BoundWorkflowAgentTarget) *gestalt.BoundWorkflowTarget {
	return &gestalt.BoundWorkflowTarget{
		Agent: agent,
	}
}

func protoWorkflowSignal(t *testing.T, id, idempotencyKey, text string) *gestalt.WorkflowSignal {
	t.Helper()
