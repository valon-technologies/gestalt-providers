...
var workflowHostFactory = func() (workflowHostClient, error) {
...
func (p *Provider) CancelRun(ctx context.Context, req *gestalt.CancelWorkflowProviderRunRequest) (*gestalt.BoundWorkflowRun, error) {
	...
	if run.Status != gestalt.WorkflowRunStatusValuePending {
		p.mu.Unlock()
		return nil, status.Errorf(codes.FailedPrecondition, "workflow run %q is %s; only pending runs can be canceled", runID, workflowRunStatusName(run.Status))
	}
	now := p.clock().UTC()
...
func (p *Provider) PublishEvent(ctx context.Context, req *gestalt.PublishWorkflowProviderEventRequest) error {
	...
	for _, trigger := range triggers {
		...
		if actorHasSubject(publishedBy) {
			...
			if ref != nil {
				record, err := executionReferenceRecordFromReference(ref)
				if err != nil {
					p.mu.RUnlock()
...
func signalRunInTransaction(ctx context.Context, stores workflowSignalTxStores, runID string, signal *gestalt.WorkflowSignal) (*gestalt.SignalWorkflowRunResponse, string, error) {
	...
	if workflowRunTerminal(run.Status) {
		return nil, "", status.Errorf(codes.FailedPrecondition, "workflow run %q is %s", runID, workflowRunStatusName(run.Status))
	}
...
func normalizeTarget(target *gestalt.BoundWorkflowTarget) (scopedTarget, error) {
	...
	if target.Agent != nil {
		agent := *target.Agent
		agent.Messages = cloneAgentMessages(agent.Messages)
		agent.ToolRefs = append([]gestalt.AgentToolRef(nil), agent.ToolRefs...)
		agent.ResponseSchema = cloneAny(agent.ResponseSchema)
		agent.Metadata = cloneAny(agent.Metadata)
		agent.OutputDelivery = cloneOutputDelivery(agent.OutputDelivery)
		agent.ModelOptions = cloneAny(agent.ModelOptions)
		agent.SessionReadyDelivery = cloneOutputDelivery(agent.SessionReadyDelivery)
		...
		normalized := cloneTarget(&gestalt.BoundWorkflowTarget{Agent: &agent})
		return scopedTarget{
	...
	plugin.Input = cloneAny(plugin.Input)
	...
	normalized := cloneTarget(&gestalt.BoundWorkflowTarget{Plugin: &plugin})
	return scopedTarget{
		OwnerKey: pluginName,
		Target:   normalized,
	}, nil
...
func normalizeWorkflowEvent(event *gestalt.WorkflowEvent, now time.Time) (*gestalt.WorkflowEvent, error) {
	...
	return &gestalt.WorkflowEvent{
		...
		Extensions:      cloneMap(event.Extensions),
	}, nil
}
...
func normalizeWorkflowSignal(signal *gestalt.WorkflowSignal, now time.Time) (*gestalt.WorkflowSignal, error) {
	...
	return &gestalt.WorkflowSignal{
		...
		CreatedBy:      cloneActor(signal.CreatedBy),
		CreatedAt:      createdAt,
		IdempotencyKey: strings.TrimSpace(signal.IdempotencyKey),
		Sequence:       signal.Sequence,
	}, nil
...
func signalInputs(records []workflowSignalRecord) []gestalt.WorkflowSignal {
	...
	for _, record := range records {
		if signal := record.signalProto(); signal != nil {
...
func eventMatchesTrigger(event *gestalt.WorkflowEvent, trigger workflowEventTriggerRecord) bool {
	...
	if strings.TrimSpace(event.Type) != trigger.MatchType {
		return false
	}
	if trigger.MatchSource != "" && strings.TrimSpace(event.Source) != trigger.MatchSource {
		return false
	}
	if trigger.MatchSubject != "" && strings.TrimSpace(event.Subject) != trigger.MatchSubject {
		return false
	}
	return true
}
...
func workflowPermissionInputs(in []gestalt.WorkflowAccessPermission) []gestalt.WorkflowAccessPermission {
	return clonePermissions(in)
}

func clonePermissions(in []gestalt.WorkflowAccessPermission) []gestalt.WorkflowAccessPermission {
	if len(in) == 0 {
		return nil
	}
...
func targetToMap(target *gestalt.BoundWorkflowTarget) map[string]any {
	if target == nil {
		return nil
	}
	if target.Plugin != nil {
		return map[string]any{"plugin": pluginTargetToMap(target.Plugin)}
	}
	if target.Agent != nil {
		agent := target.Agent
		value := map[string]any{
			...
			"model":          agent.Model,
			"prompt":         agent.Prompt,
			"messages":       cloneAny(agent.Messages),
			"toolRefs":       cloneAny(agent.ToolRefs),
			"responseSchema": cloneAny(agent.ResponseSchema),
			"metadata":       cloneAny(agent.Metadata),
			"timeoutSeconds": agent.TimeoutSeconds,
			"modelOptions":   cloneAny(agent.ModelOptions),
		}
		if agent.OutputDelivery != nil {
			value["outputDelivery"] = outputDeliveryToMap(agent.OutputDelivery)
		}
		if agent.SessionReadyDelivery != nil {
			value["sessionReadyDelivery"] = outputDeliveryToMap(agent.SessionReadyDelivery)
		}
		return map[string]any{"agent": value}
	}
	return nil
}

func targetFromMap(value map[string]any) *gestalt.BoundWorkflowTarget {
	if plugin, ok := fieldMap(value, "plugin", "Plugin"); ok {
		return &gestalt.BoundWorkflowTarget{Plugin: pluginTargetFromMap(plugin)}
	}
	if agentMap, ok := fieldMap(value, "agent", "Agent"); ok {
		agent := &gestalt.BoundWorkflowAgentTarget{
			ProviderName:   stringAnyField(agentMap, "providerName", "provider_name", "ProviderName"),
			Model:          stringAnyField(agentMap, "model", "Model"),
			Prompt:         stringAnyField(agentMap, "prompt", "Prompt"),
			Messages:       agentMessagesFromAny(anyField(agentMap, "messages", "Messages")),
			ToolRefs:       agentToolRefsFromAny(anyField(agentMap, "toolRefs", "tool_refs", "ToolRefs")),
			ResponseSchema: cloneAny(anyField(agentMap, "responseSchema", "response_schema", "ResponseSchema")),
			Metadata:       cloneAny(anyField(agentMap, "metadata", "Metadata")),
			TimeoutSeconds: int32(intFieldMulti(agentMap, "timeoutSeconds", "timeout_seconds", "TimeoutSeconds")),
			ModelOptions:   cloneAny(anyField(agentMap, "modelOptions", "model_options", "ModelOptions")),
		}
		if delivery, ok := fieldMap(agentMap, "outputDelivery", "output_delivery", "OutputDelivery"); ok {
			agent.OutputDelivery = outputDeliveryFromMap(delivery)
		}
		if delivery, ok := fieldMap(agentMap, "sessionReadyDelivery", "session_ready_delivery", "SessionReadyDelivery"); ok {
			agent.SessionReadyDelivery = outputDeliveryFromMap(delivery)
		}
...
func pluginTargetToMap(target *gestalt.BoundWorkflowPluginTarget) map[string]any {
	if target == nil {
		return nil
	}
	return map[string]any{
...
func pluginTargetFromMap(value map[string]any) *gestalt.BoundWorkflowPluginTarget {
	if value == nil {
		return nil
	}
	return &gestalt.BoundWorkflowPluginTarget{
		PluginName:     stringAnyField(value, "pluginName", "plugin_name", "PluginName"),
		Operation:      stringAnyField(value, "operation", "Operation"),
		Input:          cloneAny(anyField(value, "input", "Input")),
		Connection:     stringAnyField(value, "connection", "Connection"),
		Instance:       stringAnyField(value, "instance", "Instance"),
		CredentialMode: stringAnyField(value, "credentialMode", "credential_mode", "CredentialMode"),
	}
}

func outputDeliveryToMap(delivery *gestalt.WorkflowOutputDelivery) map[string]any {
	if delivery == nil {
		return nil
	}
	return map[string]any{
		"target":         pluginTargetToMap(delivery.Target),
		"inputBindings":  outputBindingsToAny(delivery.InputBindings),
		"credentialMode": delivery.CredentialMode,
	}
}

func outputDeliveryFromMap(value map[string]any) *gestalt.WorkflowOutputDelivery {
	if value == nil {
		return nil
	}
	var target *gestalt.BoundWorkflowPluginTarget
	if targetMap, ok := fieldMap(value, "target", "Target"); ok {
		target = pluginTargetFromMap(targetMap)
	}
	return &gestalt.WorkflowOutputDelivery{
		Target:         target,
		InputBindings:  outputBindingsFromAny(anyField(value, "inputBindings", "input_bindings", "InputBindings")),
		CredentialMode: stringAnyField(value, "credentialMode", "credential_mode", "CredentialMode"),
	}
}

func outputBindingsToAny(values []gestalt.WorkflowOutputBinding) []any {
	if len(values) == 0 {
		return nil
	}
	...
	for _, value := range values {
		out = append(out, map[string]any{
			...
			"value":      outputValueSourceToMap(value.Value),
		})
	}
	return out
}

func outputBindingsFromAny(raw any) []gestalt.WorkflowOutputBinding {
	items, ok := raw.([]any)
	if !ok {
		return nil
	}
	out := make([]gestalt.WorkflowOutputBinding, 0, len(items))
	for _, item := range items {
		...
		out = append(out, gestalt.WorkflowOutputBinding{
			InputField: stringAnyField(value, "inputField", "input_field", "InputField"),
			Value:      outputValueSourceFromMap(anyMap(anyField(value, "value", "Value"))),
		})
	}
	return out
}

func outputValueSourceToMap(source *gestalt.WorkflowOutputValueSource) map[string]any {
	if source == nil {
		return nil
	}
	return map[string]any{
		"agentOutput":    source.AgentOutput,
		"signalPayload":  source.SignalPayload,
		"signalMetadata": source.SignalMetadata,
		"literal":        cloneAny(source.Literal),
		"agentSession":   source.AgentSession,
	}
}

func outputValueSourceFromMap(value map[string]any) *gestalt.WorkflowOutputValueSource {
	if value == nil {
		return nil
	}
	return &gestalt.WorkflowOutputValueSource{
		AgentOutput:    stringAnyField(value, "agentOutput", "agent_output", "AgentOutput"),
		SignalPayload:  stringAnyField(value, "signalPayload", "signal_payload", "SignalPayload"),
		SignalMetadata: stringAnyField(value, "signalMetadata", "signal_metadata", "SignalMetadata"),
		Literal:        cloneAny(anyField(value, "literal", "Literal")),
		AgentSession:   stringAnyField(value, "agentSession", "agent_session", "AgentSession"),
	}
}

func signalToMap(signal *gestalt.WorkflowSignal) map[string]any {
	if signal == nil {
		return nil
	}
	value := map[string]any{
		...
		"payload":         cloneAny(signal.Payload),
		"metadata":        cloneAny(signal.Metadata),
		"created_by":      actorToMap(signal.CreatedBy),
		"idempotency_key": signal.IdempotencyKey,
		"sequence":        signal.Sequence,
	}
	if !signal.CreatedAt.IsZero() {
		value["created_at"] = signal.CreatedAt.UTC().Format(time.RFC3339Nano)
	}
	return value
}

func signalFromMap(value map[string]any) *gestalt.WorkflowSignal {
	if value == nil {
		return nil
	}
	createdAt := time.Time{}
	if raw := stringAnyField(value, "created_at", "createdAt", "CreatedAt"); raw != "" {
		if parsed, err := time.Parse(time.RFC3339Nano, raw); err == nil {
			createdAt = parsed.UTC()
		}
	}
	return &gestalt.WorkflowSignal{
		ID:             stringAnyField(value, "id", "ID"),
		Name:           stringAnyField(value, "name", "Name"),
		Payload:        cloneAny(anyField(value, "payload", "Payload")),
		Metadata:       cloneAny(anyField(value, "metadata", "Metadata")),
		CreatedBy:      actorFromAny(anyField(value, "created_by", "createdBy", "CreatedBy")),
		CreatedAt:      createdAt,
		IdempotencyKey: stringAnyField(value, "idempotency_key", "idempotencyKey", "IdempotencyKey"),
		Sequence:       intFieldMulti(value, "sequence", "Sequence"),
	}
}
...
func targetJSON(target *gestalt.BoundWorkflowTarget) string {
	...
	data, err := json.Marshal(targetToMap(target))
	if err != nil {
		return ""
	}
	return string(data)
...
func signalJSON(signal *gestalt.WorkflowSignal) string {
	...
	data, err := json.Marshal(signalToMap(signal))
	if err != nil {
		return ""
	}
	return string(data)
...
func targetFromRecordValue(recordKind, id string, raw any) (*gestalt.BoundWorkflowTarget, error) {
	...
	if err := validateTargetMap(data); err != nil {
		return nil, fmt.Errorf("%s %q invalid target_json: %w", recordKind, id, err)
	}
	target := targetFromMap(data)
...
func validateTargetMap(value map[string]any) error {
	if err := rejectUnknownFields("target", value, "plugin", "Plugin", "agent", "Agent"); err != nil {
		return err
	}
	if plugin, ok := fieldMap(value, "plugin", "Plugin"); ok {
		return validatePluginTargetMap("target.plugin", plugin)
	}
	if agentMap, ok := fieldMap(value, "agent", "Agent"); ok {
		if err := rejectUnknownFields(
			...
			"outputDelivery", "output_delivery", "OutputDelivery",
			"sessionReadyDelivery", "session_ready_delivery", "SessionReadyDelivery",
		); err != nil {
			return err
		}
		for _, keys := range [][]string{
			...
			if delivery, ok := fieldMap(agentMap, keys...); ok {
				if err := validateOutputDeliveryMap("target.agent."+keys[0], delivery); err != nil {
					return err
				}
			}
		}
...
func validatePluginTargetMap(context string, value map[string]any) error {
...
func validateOutputDeliveryMap(context string, value map[string]any) error {
	if err := rejectUnknownFields(
		context,
		value,
		"target", "Target",
		"inputBindings", "input_bindings", "InputBindings",
		"credentialMode", "credential_mode", "CredentialMode",
	); err != nil {
		return err
	}
	if target, ok := fieldMap(value, "target", "Target"); ok {
		if err := validatePluginTargetMap(context+".target", target); err != nil {
			return err
		}
	}
	bindings, ok := anyField(value, "inputBindings", "input_bindings", "InputBindings").([]any)
	if !ok {
		return nil
	}
	for _, binding := range bindings {
		...
		if err := rejectUnknownFields(context+".inputBindings", bindingMap, "inputField", "input_field", "InputField", "value", "Value"); err != nil {
			return err
		}
		if valueMap := anyMap(anyField(bindingMap, "value", "Value")); valueMap != nil {
			if err := rejectUnknownFields(
				context+".inputBindings.value",
				valueMap,
				"agentOutput", "agent_output", "AgentOutput",
				"signalPayload", "signal_payload", "SignalPayload",
...
func rejectUnknownFields(context string, value map[string]any, allowed ...string) error {
	if value == nil {
		return nil
	}
	...
	for field := range value {
		if _, ok := known[field]; !ok {
			return fmt.Errorf("%s has unknown field %q", context, field)
		}
	}
	return nil
}
...
func fieldMap(value map[string]any, keys ...string) (map[string]any, bool) {
	raw := anyField(value, keys...)
	if raw == nil {
		return nil, false
	}
	out := anyMap(raw)
...
func anyField(value map[string]any, keys ...string) any {
...
func stringAnyField(value map[string]any, keys ...string) string {
	return stringValue(anyField(value, keys...))
}

func intFieldMulti(value map[string]any, keys ...string) int64 {
	raw := anyField(value, keys...)
	if raw == nil {
		return 0
	}
...
func cloneMap(value map[string]any) map[string]any {
	...
	out, ok := cloneAny(value).(map[string]any)
	if !ok {
		return nil
	}
	return out
...
func agentToolRefsFromAny(raw any) []gestalt.AgentToolRef {
	items, ok := raw.([]any)
	if !ok {
		return nil
	}
	out := make([]gestalt.AgentToolRef, 0, len(items))
	for _, item := range items {
		value := anyMap(item)
		if value == nil {
			continue
		...
		out = append(out, gestalt.AgentToolRef{
			Plugin:      stringAnyField(value, "plugin", "Plugin"),
			Operation:   stringAnyField(value, "operation", "Operation"),
			Connection:  stringAnyField(value, "connection", "Connection"),
			Instance:    stringAnyField(value, "instance", "Instance"),
			Title:       stringAnyField(value, "title", "Title"),
			Description: stringAnyField(value, "description", "Description"),
			System:      stringAnyField(value, "system", "System"),
		})
	}
	return out
}

func agentMessagesFromAny(raw any) []gestalt.AgentMessage {
	items, ok := raw.([]any)
	if !ok {
		return nil
	}
	out := make([]gestalt.AgentMessage, 0, len(items))
	for _, item := range items {
		value := anyMap(item)
		if value == nil {
			continue
		...
		out = append(out, gestalt.AgentMessage{
			Role:     stringAnyField(value, "role", "Role"),
			Text:     stringAnyField(value, "text", "Text"),
			Metadata: anyMap(anyField(value, "metadata", "Metadata")),
		})
	}
	return out
}
...
func (r workflowScheduleRecord) toProto() (*gestalt.BoundWorkflowSchedule, error) {
...
func (r workflowEventTriggerRecord) toProto() (*gestalt.BoundWorkflowEventTrigger, error) {
...
func (r workflowRunRecord) toProto() (*gestalt.BoundWorkflowRun, error) {
...
func (r workflowSignalRecord) signalProto() *gestalt.WorkflowSignal {
	input := cloneSignal(r.Signal)
	if input == nil {
...
func executionReferenceRecordFromReference(ref *gestalt.WorkflowExecutionReference) (workflowExecutionReferenceRecord, error) {
	if ref == nil {
		return workflowExecutionReferenceRecord{}, errors.New("reference is required")
	}
	target, err := normalizeTarget(ref.Target)
	if err != nil {
	...
	if ref.RevokedAt != nil && !ref.RevokedAt.IsZero() {
		record.RevokedAt = timePtr(*ref.RevokedAt)
	}
	return record, nil
}
...
func (r workflowExecutionReferenceRecord) toProto() (*gestalt.WorkflowExecutionReference, error) {
...
func executionReferenceRunAsJSON(value *gestalt.WorkflowRunAsSubject) (string, error) {
	...
	data, err := json.Marshal(map[string]any{
		...
		"auth_source":  normalized.AuthSource,
	})
	if err != nil {
		return "", err
	}
...
func executionReferenceRunAsFromJSON(raw string) (*gestalt.WorkflowRunAsSubject, error) {
	...
	out := &gestalt.WorkflowRunAsSubject{
		SubjectID:   stringAnyField(data, "subject_id", "subjectId", "SubjectID"),
		SubjectKind: stringAnyField(data, "subject_kind", "subjectKind", "SubjectKind"),
		DisplayName: stringAnyField(data, "display_name", "displayName", "DisplayName"),
		AuthSource:  stringAnyField(data, "auth_source", "authSource", "AuthSource"),
	}
...
func cloneOutputDelivery(delivery *gestalt.WorkflowOutputDelivery) *gestalt.WorkflowOutputDelivery {
	if delivery == nil {
		return nil
	}
	out := &gestalt.WorkflowOutputDelivery{
		CredentialMode: delivery.CredentialMode,
	}
	if delivery.Target != nil {
		target := *delivery.Target
		target.Input = cloneAny(target.Input)
		out.Target = &target
	}
	if len(delivery.InputBindings) > 0 {
		out.InputBindings = make([]gestalt.WorkflowOutputBinding, 0, len(delivery.InputBindings))
		for _, binding := range delivery.InputBindings {
			out.InputBindings = append(out.InputBindings, gestalt.WorkflowOutputBinding{
				InputField: binding.InputField,
				Value:      cloneOutputValueSource(binding.Value),
			})
		}
	}
	return out
...
func cloneOutputValueSource(source *gestalt.WorkflowOutputValueSource) *gestalt.WorkflowOutputValueSource {
	if source == nil {
		return nil
	}
	return &gestalt.WorkflowOutputValueSource{
...
func cloneAgentMessages(values []gestalt.AgentMessage) []gestalt.AgentMessage {
	if len(values) == 0 {
		return nil
	}
	out := make([]gestalt.AgentMessage, 0, len(values))
	for _, value := range values {
		out = append(out, gestalt.AgentMessage{
			Role:     value.Role,
			Text:     value.Text,
			Parts:    cloneAgentMessageParts(value.Parts),
...
func cloneAgentMessageParts(values []gestalt.AgentMessagePart) []gestalt.AgentMessagePart {
	if len(values) == 0 {
		return nil
	}
	out := make([]gestalt.AgentMessagePart, 0, len(values))
	for _, value := range values {
		part := gestalt.AgentMessagePart{
			Type: value.Type,
			Text: value.Text,
			JSON: cloneMap(value.JSON),
		}
		if value.ToolCall != nil {
			part.ToolCall = &gestalt.AgentMessagePartToolCall{
				ID:        value.ToolCall.ID,
				ToolID:    value.ToolCall.ToolID,
				Arguments: cloneMap(value.ToolCall.Arguments),
			}
		}
		if value.ToolResult != nil {
			part.ToolResult = &gestalt.AgentMessagePartToolResult{
				ToolCallID: value.ToolResult.ToolCallID,
				Status:     value.ToolResult.Status,
				Content:    value.ToolResult.Content,
				Output:     cloneMap(value.ToolResult.Output),
			}
		}
		if value.ImageRef != nil {
			part.ImageRef = &gestalt.AgentMessagePartImageRef{
				URI:      value.ImageRef.URI,
				MimeType: value.ImageRef.MimeType,
			}
		}
		out = append(out, part)
	}
