...
type workflowObjectStore interface {
	...
	OpenKeyCursor(ctx context.Context, r *gestalt.KeyRange, dir gestalt.CursorDirection) (workflowCursor, error)
}

type workflowIndex interface {
	GetAll(ctx context.Context, r *gestalt.KeyRange, values ...any) ([]gestalt.Record, error)
	Count(ctx context.Context, r *gestalt.KeyRange, values ...any) (int64, error)
	OpenKeyCursor(ctx context.Context, r *gestalt.KeyRange, dir gestalt.CursorDirection, values ...any) (workflowCursor, error)
}
...
func (s sdkWorkflowStore) OpenKeyCursor(ctx context.Context, r *gestalt.KeyRange, dir gestalt.CursorDirection) (workflowCursor, error) {
	cur, err := s.ObjectStoreClient.OpenKeyCursor(ctx, r, dir)
	if err != nil {
		return nil, err
	}
	return sdkWorkflowCursor{cur}, nil
...
func (idx sdkWorkflowIndex) OpenKeyCursor(ctx context.Context, r *gestalt.KeyRange, dir gestalt.CursorDirection, values ...any) (workflowCursor, error) {
	cur, err := idx.IndexClient.OpenKeyCursor(ctx, r, dir, values...)
	if err != nil {
		return nil, err
	}
	return sdkWorkflowCursor{cur}, nil
