...
import type { GitHubActionPreferenceTargets, Integration } from "../src/lib/api";
...
const GITHUB_INTEGRATION: Integration = {
  name: "github",
  displayName: "GitHub",
  connections: [{ name: "default", displayName: "Default", authTypes: ["oauth"] }],
};
...
test.describe("Integrations", () => {
  ...
  test("github settings exposes current-user action preferences", async ({
    authenticatedPage,
  }) => {
    const page = authenticatedPage;
    const requests: Array<Record<string, unknown>> = [];
    const targets: GitHubActionPreferenceTargets = {
      identity: {
      ...
      repositories: [
        {
          ...
          html_url: "https://github.com/valon-technologies/toolshed",
          installation_id: 99,
          controls: [
            {
              id: "github-pull-request-review:allow_code_review_comments",
              policy_id: "github-pull-request-review",
              field: "allow_code_review_comments",
              label: "Automatic PR code review",
              description: "Inline review comments on your PRs.",
              config_default: true,
              stored: null,
              effective: true,
            },
            {
              id: "github-pull-request-review:allow_self_fix",
              policy_id: "github-pull-request-review",
              field: "allow_self_fix",
              label: "Self-fix from PR comments",
              description: "Fix commits from PR comments.",
              config_default: true,
              stored: false,
              effective: false,
            },
    ...
    await mockIntegrations(page, [withConnectedConnection(GITHUB_INTEGRATION)]);
    await page.route(
      "**/api/v1/github/actionPreferences.listTargets",
      async (route) => {
        await route.fulfill({ json: { data: targets } });
      },
    );
    await page.route("**/api/v1/github/actionPreferences.set", async (route) => {
    ...
    await expect(dialog.getByText("GitHub Bot Actions")).toBeVisible();
    await expect(dialog.getByText("valon-technologies/toolshed")).toBeVisible();
    await dialog
      .getByRole("group", {
        name: /Automatic PR code review for valon-technologies\/toolshed/,
      })
      .getByRole("button", { name: "Off" })
      .click();
    await dialog
      .getByRole("group", {
        name: /Self-fix from PR comments for valon-technologies\/toolshed/,
      })
  ...
  test("github settings deletes preference records when reset to defaults", async ({
    authenticatedPage,
  }) => {
    const page = authenticatedPage;
    ...
    await page.route("**/api/v1/github/actionPreferences.delete", async (route) => {
  ...
  test("github settings hides preferences when provider support is unavailable", async ({
    authenticatedPage,
  }) => {
    const page = authenticatedPage;
    ...
    await page.route(
      "**/api/v1/github/actionPreferences.listTargets",
      async (route) => {
        await route.fulfill({ status: 412, json: { error: "not configured" } });
      },
    );
    ...
    await expect(dialog.getByText("GitHub Bot Actions")).toHaveCount(0);
  });

  test("github settings hides preferences when provider operation is missing", async ({
    authenticatedPage,
  }) => {
    const page = authenticatedPage;
    ...
    await expect(page.getByRole("dialog").getByText("GitHub Bot Actions")).toHaveCount(0);
  });

  test("github settings hides preferences when no targets are configurable", async ({
    authenticatedPage,
  }) => {
    const page = authenticatedPage;
    ...
    await page.route(
      ...
      async (route) => {
        await route.fulfill({
          json: {
            data: {
              identity: { identity_kind: "external_subject_id" },
              repositories: [],
            },
          },
        });
