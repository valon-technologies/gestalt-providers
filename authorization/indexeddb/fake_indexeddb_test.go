...
func (db *fakeIndexedDB) createdStoreNames() []string {
	db.mu.Lock()
	defer db.mu.Unlock()
	return append([]string(nil), db.createdStores...)
}

func (db *fakeIndexedDB) storeSchema(name string) (gestalt.ObjectStoreSchema, bool) {
	db.mu.Lock()
	defer db.mu.Unlock()
...
func seedAuthorizationStoresOnConn(ctx context.Context, db indexedDBConn) error {
	for _, def := range []struct {
		...
		if err := db.CreateObjectStore(ctx, def.name, def.schema); err != nil {
			return err
		}
	}
	return nil
